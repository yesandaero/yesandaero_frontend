...
API 명세서 기반으로 프론트엔드를 구현한다. **실제 API 연동 모드**로 동작하므로 백엔드 서버가 떠 있어야 한다.
...
**실제 API 연동 모드로 동작한다.** 모든 화면이 `VITE_API_BASE_URL`의 백엔드를 직접 호출하므로,
서버가 떠 있어야 정상 동작한다. 서버 주소가 다르면 이 값을 바꾼다.
(백엔드가 없으면 요청이 실패하고 화면에는 에러 토스트가 뜬다 — 크래시하지는 않는다.)
...
│
├─ api/                    # ── 통신 레이어 ──
│  ├─ client.ts            # fetch 래퍼, 토큰 관리, 401 자동 재발급, 에러 파싱
...
- ✅ **제휴 5종 전부 명세서에 존재 확인 (백엔드 "완료")** — 목록 조회 / 요청 / 수락 / 거절 / 해지.
  요청·응답·에러가 명세서와 일치함을 검증 완료. 이전에 "표에 없다"고 보류했던 건 표가 덜 채워져 있던 것.

### 아직 확인이 필요한 항목

최신 API 표(Notion)와 현재 프론트 구현 사이에 남은 불일치. **임의로 제거하지 않고 유지** 중:

- **쿠폰 템플릿 생성(POST `/coupon-templates`)** — 표엔 GET만 있음. 생성 UI 유지 중.
- **가게 등록(POST `/stores`) / 내 가게 조회(GET `/stores/me`)** — 표엔 수정·상세조회만. 온보딩·조회 유지 중.
- **쿠폰 사용 코드(`/coupons/{id}/use-token`)** vs 표의 **쿠폰 사용(`/coupons/{id}/use`)** — 이름/담당 상이. 콘솔 UI엔 미사용(고객 앱 영역).
- **제휴 요청 409 에러 코드 미상** — "이미 진행 중이거나 체결된 제휴입니다"(UNIQUE 제약)의 code가 카탈로그에 없음.
  번호 체계상 `PTN_409_01`로 추정되나 미확인이라 **추가하지 않음**. 미등록 코드는 백엔드 `message`로 폴백되므로 동작엔 문제 없음.
...
- [ ] 위 §10 불일치 팀 확인 → 범위 확정
- [ ] 실서버 주소를 `VITE_API_BASE_URL`에 반영하고 auth/제휴부터 실연동 테스트 (백엔드 "완료" 상태)
...
- [x] ~~백엔드 연동 완료 후 `api/mock/*` 및 각 `api/*.ts`의 `USE_MOCK` 분기 제거~~ → 목업 제거 완료, 실 API 연동 모드로 전환됨
