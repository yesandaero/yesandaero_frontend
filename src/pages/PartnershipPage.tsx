...
function statusMeta(status: PartnershipStatus): {
  label: string;
  badgeClass: string;
} {
  switch (status) {
...
function PartnershipRow({ p }: { p: Partnership }) {
  const { acceptPartnership, rejectPartnership, requestTerminatePartnership } =
    useApp();
    ...
    <div className="board-row">
      <div className="board-name">
        ...
        <span className="empty-inline">
          {categoryLabel(p.partnerStore.category)}
        </span>
        <span className="empty-inline">
          {p.direction === "SENT" ? "내가 요청" : "요청 받음"}
        </span>
      </div>
      <div className="promo-right">
        {isReceivedPending && (
          <>
            <button
              ...
              onClick={() => rejectPartnership(p.partnershipId)}
            >
              거절
            </button>
            <button
              className="btn btn-primary btn-sm"
              onClick={() => acceptPartnership(p.partnershipId)}
            >
              수락
            </button>
          </>
        ...
        {p.status === "ACCEPTED" && (
          <button
            className="btn btn-outline btn-sm"
            onClick={() => requestTerminatePartnership(p.partnershipId)}
          >
            해지
          </button>
        )}
...
export function PartnershipPage() {
  ...
  const received = partnerships.filter(
    (p) => p.status === "PENDING" && p.direction === "RECEIVED",
  );
  const sent = partnerships.filter(
    (p) => p.status === "PENDING" && p.direction === "SENT",
  );
      ...
      {partnershipsLoading ? (
        ...
        <>
          {received.length > 0 && (
            <>
              <div className="card-head" style={{ marginBottom: 6 }}>
                <h2 style={{ fontSize: 14 }}>받은 요청</h2>
              </div>
              {received.map((p) => (
                <PartnershipRow key={p.partnershipId} p={p} />
              ))}
            </>
          )}
          {sent.length > 0 && (
          ...
          {history.length > 0 && (
            <>
              <div className="card-head" style={{ marginBottom: 6 }}>
                <h2 style={{ fontSize: 14, marginTop: "12px" }}>지난 이력</h2>
              </div>
              {history.map((p) => (
                <PartnershipRow key={p.partnershipId} p={p} />
              ))}
        ...
        <div className="empty-state">
          <div className="empty-icon">
            <Icon name="partnership" size={27} />
          </div>
          아직 제휴가 없어요.
          <br />
          위의 '제휴 요청' 버튼으로 다른 가게에 제휴를 요청해보세요.
        </div>
      )}
    </div>
  );
